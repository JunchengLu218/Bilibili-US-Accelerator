...
最后一条 `generic` 是用户手动运行的测速入口，超时设为 120 秒。0.1.2 为它显式添加 `img-url=atom.system` 和 `enable=true`，使它能注册为 Loon 节点长按菜单里的可见操作。`.system` 后缀表示使用系统 SF Symbol；不带后缀时 Loon 可能把值当成无效图片地址。0.1.3 又为 generic 的 `$done()` 增加了 `title` 和 `htmlMessage`，因此成功排名、无样本、全部失败和异常都会直接显示在运行结果页，不再只显示 `empty content`。按默认四候选、一轮、3 秒单项超时，理论最坏网络等待约 12 秒；三轮、5 秒单项超时最坏约 60 秒。

0.1.4 为实机缓存交接增加了诊断信息。测速完成页会显示缓存网络键和设置指纹；`LogLevel=INFO` 时，后续视频请求会输出缓存状态、当前/已存指纹，以及最近一次成功测速的网络/profile 摘要。摘要不保存签名 URL、Cookie 或请求头，只用于区分网络键变化、参数变化、过期和入口间存储不可见。
